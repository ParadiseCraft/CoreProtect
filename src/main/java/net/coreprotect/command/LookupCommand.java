...
public class LookupCommand {
    public static void runCommand(CommandSender player, Command command, boolean permission, String[] args) {
        ...
        if (!allPermission) {
            if ((argAction.isEmpty() || argAction.contains(LookupActions.BLOCK_BREAK) || argAction.contains(LookupActions.BLOCK_PLACE)) && !player.hasPermission("coreprotect.lookup.block")) {
        ...
        if (startTime <= 0 && type == 4 && (argBlocks.size() > 0 || argUsers.size() > 0)) {
        ...
        if (type == 1 || type == 2 || type == 3 || type == 7 || type == 8 || type == 9) {
            ...
            if (g == 1 && (argBlocks.size() > 0 || argUsers.size() > 0 || (argUsers.size() == 0 && argRadius != null))) {
